...
class ExpertsTests {
    companion object {
        ...
        lateinit var testExpert1: Expert
        lateinit var testExpert2: Expert
    }
    ...
    fun populateDb() {
        ...
        testExpert1 = Expert("John", "Smith")
        testExpert2 = Expert("Jack", "Smith")
        expertRepository.save(testExpert1)
    }

    @Test
    fun getExpert() {
        val res = restTemplate.exchange("/API/experts/${testExpert1.id}", HttpMethod.GET, null, typeReference<ExpertDTO>())

        Assertions.assertEquals(HttpStatus.OK, res.statusCode)
        Assertions.assertEquals(testExpert1.toDTO(), res.body)
    }

    @Test
    fun expertNotFound() {
        // using textExpert2.id, which is 0 as it has not been saved
        val res = restTemplate.exchange("/API/experts/${testExpert2.id}", HttpMethod.GET, null, typeReference<Unit>())
    ...
    @Test
    fun createExpert() {
        val requestEntity = HttpEntity(testExpert2.toNewDTO())
        ...
        val createdExpert = testExpert2.apply { id = res.body!!.id }.toDTO()
    ...
    fun creatingAlreadyExistingExpertShouldNotFail() {
        val requestEntity = HttpEntity(testExpert1.toNewDTO())
        val res = restTemplate.exchange("/API/experts", HttpMethod.POST, requestEntity, typeReference<ExpertDTO>())

        // textExpert2 with newly autogenerated id
        val createdExpert = testExpert1.apply { id = res.body!!.id }.toDTO()
    ...
    fun editExpert() {
        ...
        val editedExpert = ExpertDTO(testExpert1.id, testExpert2.firstName, testExpert2.lastName, testExpert2.tickets.map { it.toDTO() })
